...
bowden_extraheight = 5;
extruder_distance = 20;	// distance between 2 hotends. Set to 0 for only 1 hotend
hotend_z_offset = 4;	// shift hotend holes in Z-direction to make it fit on the carriage if needed.
...
difference(){
	...
	//translate([0,2,0]) #cylinder(200,1.5,1.5,$fn=50);
...
for (x = [-extruder_distance/2, extruder_distance/2]) // two iterations, x = -extruder_distance/2, z = extruder_distance/2
{
    translate([x, 0, 0])
	{

			// Extra cube
			translate([motor_mount_translation[0]-gear_separation-filament_feed_hole_offset-wade_block_width/2,0,wade_block_depth*(1-roundness)])
			cube([wade_block_width,12+bowden_extraheight,wade_block_depth*roundness]);
	
			difference(){
			// round piece
			translate([motor_mount_translation[0]-gear_separation-filament_feed_hole_offset-wade_block_width+wade_block_width,
				0,wade_block_depth*(1-roundness)])
			rotate([-90,0,0])
			cylinder(r=wade_block_width/2,h=12+bowden_extraheight,$fn=200);

			translate([motor_mount_translation[0]-gear_separation-filament_feed_hole_offset-wade_block_width/2,-1,-wade_block_depth/2])
			cube([wade_block_width,14+bowden_extraheight,wade_block_depth/2]);
    ...
    translate([x, 0, 0])
	{
		...
		translate(motor_mount_translation){
			translate([-gear_separation,0,0]){
				...
				rotate([90,0,0])
				rotate(360/16)
				cylinder(r=4.3,h=wade_block_depth*3,center=true,$fn=200);	
...
module wildseyed_mount_holes(insulator_d=12.7){  
	...
	for (hole=[-1,1])
	rotate(90,[1,0,0])
	translate([hole*(extruder_recess_d/2-1.5),3+1.5,-wade_block_depth/2-1-hotend_z_offset])
	cylinder(r=1.5,h=wade_block_depth+2+hotend_z_offset,$fn=10);
}
