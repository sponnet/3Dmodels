...
module vertical_bearing_base(height){
 translate(v=[-bearing_size/2-2,-bearing_size/2,0]) cube(size = [bearing_size/2+2,bearing_size,height], center = false);
 cylinder(h = height, r=bearing_size/2, $fn = 60);
}

module vertical_bearing_holes(height){
  translate(v=[0,0,-1]) cylinder(h = height+2, r=bearing_diameter/2, $fn = 60);
  rotate(a=[0,0,-60]) translate(v=[10,0,(height/2)+0.1]) cube(size = [10,1,height], center = true);
}
